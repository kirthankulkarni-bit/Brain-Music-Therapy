...
```bash
python src/live_music.py --participant P02 --condition sham --yoke-from sessions/P01_20260824_101500 --duration 20
```
...
**Yoke to a DIFFERENT participant, never to the same one.** The study is a crossover, so
every participant runs both arms — which makes self-yoking the obvious shortcut and the
wrong one. A participant's own earlier schedule was generated by their own brain
dynamics, and people are consistent enough between days that it may still partially
track them, breaking the contingency only halfway and biasing the contrast toward the
null. `live_music.py` prints a loud SELF-YOKED warning if you do it; do not proceed past
it.

### Crossover schedule

Each participant completes both arms:

| | |
|---|---|
| sessions per participant | 2 (one adaptive, one sham) |
| washout between them | **≥ 48 hours** |
| order | counterbalanced — half adaptive-first, assigned before enrolment |
| sham source | a different participant's adaptive session |

Keep a written allocation table before enrolment starts: participant, order, and which
session their sham is yoked to. Deciding a yoke source on the day, with the participant
in the room, is how a self-yoke or a duplicate source happens.

**Seeding.** A sham-first participant needs an existing adaptive session from someone
else. Seed from the operator's own post-fix pilot, or run the first participant
adaptive-first. Record which.
