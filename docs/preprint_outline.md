...
### §9 Planned study
Point at `docs/analysis_plan.md` and the power analysis. Stating the design in advance,
in a preprint that cannot report its outcome, is a credibility asset rather than an
admission.
...
| task | why |
|---|---|
| **Decide independent vs crossover** | §9 cannot be written without it |
| **Justify the 0.3 z smallest effect** | a sample size from an arbitrary effect is not a sample size |
