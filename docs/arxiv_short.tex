...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{fig6-estimator-tradeoff.png}
\caption{Every estimator scored on detection latency (lower better) and information per
minute (higher better). The deployed configuration sits at the bottom right: the shaded
region is strictly better on both axes at once, and 8 of the 9 alternatives fall in it.
A trade-off would put the deployed point on the upper-right frontier; it is not there.}
\label{fig:frontier}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\linewidth]{fig2-autocorrelation.png}
\caption{Autocorrelation of the arousal index against lag, from a $20$-minute session. The
index decorrelates at about $9$~s, which is roughly the smoother's time constant times
three---so the effective sample size is set by a design parameter we chose, not by the
participant.}
\label{fig:autocorr}
\end{figure}
...
significant alpha rise---$1.34\times$ geometric, $d = 0.53$,
$p = 2.3 \times 10^{-6}$, and $1.41\times$/$d = 0.61$ if the declared $3$~s settle after each
instruction is excluded---but
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{fig0-alpha-validation.png}

\vspace{0.6em}
\includegraphics[width=\linewidth]{fig0b-alpha-validation-af78.png}
\caption{Both validations, reported together rather than one substituted for the other.
\emph{Top:} the temporal pair TP9/TP10, which establishes that the rig records cortex.
\emph{Bottom:} AF7/AF8, the channels the arousal index is actually computed from, where the
same manipulation gives roughly half the effect. Neither panel alone is the honest summary:
the first says nothing about the electrodes in use, and the second would discard the
evidence that the instrument works at all. The gap between them is the finding. The flat
span near $300$~s in the lower panel is a stream dropout.}
\label{fig:alpha}
\end{figure}
...
which checks it against the value asserted here: $46$ claims, all reproducing. The practice
...
\url{https://github.com/kirthankulkarni-bit/Brain-Music-Therapy}.
...
\noindent which reads the recorded sessions and benchmark logs rather than any stored
summary, and fails if a paper value and a recomputed value disagree. Figures are produced by
\texttt{scripts/make\_figures.py} from the same artefacts. The audio library is excluded from
the repository for size; its manifest is committed and \texttt{scripts/build\_library.py}
regenerates it.
...
\paragraph{Reproducibility.} $46$ of the numbers in this paper are regenerated from artefacts
