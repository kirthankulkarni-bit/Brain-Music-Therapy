...
Expected: **105 passed, 0 failed** and **39 claims reproduce**. Takes ~100 s; `--quick` cuts it to ~65 s by skipping everything that recomputes from raw data, and says so rather than looking clean. If either disagrees,
...
| `scripts/run_tests.py` | 105 checks, one command |
