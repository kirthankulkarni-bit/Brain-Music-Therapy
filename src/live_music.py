...
def eeg_worker(args, state: SessionState, logger: SessionLogger) -> None:
    ...
    try:
        yoked_prompts = (_load_yoked_prompts(args.yoke_from, args.participant)
                         if args.yoke_from else None)
        if yoked_prompts:
...
def _load_yoked_prompts(session_dir: str, participant: Optional[str] = None) -> list[tuple[float, str]]:
    """
...
def _warn_if_self_yoked(session_dir: str, participant: Optional[str]) -> None:
    """
    A participant's sham must replay SOMEONE ELSE'S schedule.

    The study uses a crossover design, so each participant runs both arms. The
    tempting shortcut is to yoke their sham to their own earlier adaptive session -
    it is acoustically matched to what they personally experienced, and the music no
    longer responds to them, so it looks like a valid sham.

    It is a weak one. That schedule was generated BY their own brain dynamics, and
    people are somewhat consistent between days. A participant who relaxes on a
    similar trajectory on both days receives music that happens to track them, so the
    contingency is broken only partially and the arms differ by less than the design
    intends. The effect is a bias toward the null, which is the direction that quietly
    wastes a study rather than the one that produces a false positive.

    Cross-yoking keeps the crossover's statistical advantage - participant offsets
    still cancel in the paired difference - while making the sham's schedule genuinely
    independent of the person hearing it.
    """
    if not participant:
        return
    ...
    try:
        with open(manifest_path, encoding="utf-8") as fh:
            source_participant = json.load(fh).get("participant_id")
    except Exception:  # noqa: BLE001 - a diagnostic must never break the session
        return

    if source_participant and str(source_participant) == str(participant):
        print()
        print("[eeg] " + "!" * 66)
        print(f"[eeg] WARNING: SELF-YOKED. {participant} is being yoked to their own session.")
        print("[eeg]   The schedule was generated by this participant's own brain, and")
        print("[eeg]   people are consistent enough between days that it may still track")
        print("[eeg]   them. That breaks the contingency only partially and biases the")
        print("[eeg]   contrast toward the null.")
        print("[eeg]   Yoke to a DIFFERENT participant's adaptive session. The crossover")
        print("[eeg]   keeps its advantage either way - offsets cancel in the paired")
        print("[eeg]   difference regardless of whose schedule the sham replays.")
        print("[eeg] " + "!" * 66)
        print()
